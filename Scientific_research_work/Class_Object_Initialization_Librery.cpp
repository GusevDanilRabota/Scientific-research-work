...
class ADC{
private: /* ДЕЛАЕМ ИХ ДОСТУПНЫМИ ТОЛЬКО ВНУТРИ КЛАССА, ЧТОБЫ ИСПОЛЬЗОВАТЬ ТОЛЬКО ЧЕРЕЗ КОНСТРУКТОР */
    std::string name;
    u_int8_t pin;
...
public:
    ...
    /* Предопределение функций, на случай, если нужно будет сделать привватными */
...
/* ФУНКЦИЯ ЧТЕНИЯ */
u_int32_t ADC::read(){ /* Принятые данные сохраняются во внутреннем буфере Rx, в то время как при передаче данные предварительно сохраняются во внутренний буфер Tx,после чего они передаются. 
...
int main(){
    ...
    while (true)
    {
        if(spi_is_readable(PORT_SPI)){ /* ПРОВЕРЯЕМ НАЛИЧИЕ ДАННЫХ */
            for (ADC_name = {ADC_1, ADC_2, ADC_1})
            {
                ADC_name.read();
            }
            for (ADC_name = {ADC_1, ADC_2, ADC_1})
            {
                spi_write_blocking(PORT_SPI, 0, ADC_name.in_buf, SPI_RW_LEN);
            }    
        }
    }
